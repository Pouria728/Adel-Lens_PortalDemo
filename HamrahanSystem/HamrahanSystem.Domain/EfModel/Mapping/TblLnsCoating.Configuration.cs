...
{
    /// <summary>
    /// There are no comments for TblLnsCoatingConfiguration in the schema.
    /// </summary>
    public partial class TblLnsCoatingConfiguration : IEntityTypeConfiguration<TblLnsCoating>
    {
        /// <summary>
        /// There are no comments for Configure(EntityTypeBuilder<TblLnsCoating> builder) method in the schema.
        /// </summary>
        public void Configure(EntityTypeBuilder<TblLnsCoating> builder)
        {
            builder.ToTable(@"Tbl_Lns_Coating", @"dbo");
            builder.Property(x => x.CoatingId).HasColumnName(@"CoatingId").HasColumnType(@"int").IsRequired().ValueGeneratedOnAdd().HasPrecision(10, 0);
            ...
            builder.HasKey(@"CoatingId");
            builder.HasMany(x => x.TblLnsBrandCoatings).WithOne(op => op.TblLnsCoating).HasForeignKey(@"CoatingId").IsRequired(false);
            builder.HasMany(x => x.TblLnsOrders).WithOne(op => op.TblLnsCoating).HasForeignKey(@"CoatingId").IsRequired(false);
        ...
        partial void CustomizeConfiguration(EntityTypeBuilder<TblLnsCoating> builder);
