...
{
    /// <summary>
    /// There are no comments for TblLnsBrandConfiguration in the schema.
    /// </summary>
    public partial class TblLnsBrandConfiguration : IEntityTypeConfiguration<TblLnsBrand>
    {
        /// <summary>
        /// There are no comments for Configure(EntityTypeBuilder<TblLnsBrand> builder) method in the schema.
        /// </summary>
        public void Configure(EntityTypeBuilder<TblLnsBrand> builder)
        {
            builder.ToTable(@"Tbl_Lns_Brand", @"dbo");
            builder.Property(x => x.BrandId).HasColumnName(@"BrandId").HasColumnType(@"int").IsRequired().ValueGeneratedOnAdd().HasPrecision(10, 0);
            ...
            builder.Property(x => x.IsStock).HasColumnName(@"IsStock").HasColumnType(@"bit").IsRequired().ValueGeneratedNever().HasDefaultValueSql(@"0");
            builder.Property(x => x.IsStockGranty).HasColumnName(@"IsStockGranty").HasColumnType(@"bit").IsRequired().ValueGeneratedNever().HasDefaultValueSql(@"0");
            ...
            builder.HasMany(x => x.TblLnsBrandCoatings).WithOne(op => op.TblLnsBrand).HasForeignKey(@"BrandId").IsRequired(false);
            builder.HasMany(x => x.TblLnsBrandLensTypes).WithOne(op => op.TblLnsBrand).HasForeignKey(@"BrandId").IsRequired(false);
            builder.HasMany(x => x.TblLnsBrandLensTypeRs).WithOne(op => op.TblLnsBrand).HasForeignKey(@"BrandId").IsRequired(false);
            builder.HasMany(x => x.TblLnsOrders).WithOne(op => op.TblLnsBrand).HasForeignKey(@"BrandId").IsRequired(false);
            builder.HasMany(x => x.TblLnsOrderItems).WithOne(op => op.TblLnsBrand).HasForeignKey(@"BrandId").IsRequired(false);
        ...
        partial void CustomizeConfiguration(EntityTypeBuilder<TblLnsBrand> builder);
